...
class CallPoliceOnBehalfOfDialog extends CancelAndHelpDialog {
    ...
    async getLocationStep(stepContext) {
        return await stepContext.prompt(TEXT_PROMPT, 'Please send us your current location. (Not live location)');
    }
    ...
    async getLocation(stepContext) {
        if (stepContext.activity.attachments && stepContext.activity.attachments.length > 0) {
            for (const attachment of stepContext.activity.attachments) {
